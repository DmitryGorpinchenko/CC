...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
    ...
    p_expr_token stack = NULL, token = NULL, tmp = NULL;
    while (token = dequeue(pqueue_infix)) {
        if (token->type == OPERAND) {
            enqueue(&postfix, token);
        } else if (token->type == OPERATOR) {
            while(stack && stack->type == OPERATOR) {
                unsigned int top_prec = op_precedences[stack->value.op_code];
                unsigned int cur_prec = op_precedences[token->value.op_code];
                enum assoc ass = op_associativity[top_prec];
                if(!(top_prec > cur_prec || (top_prec == cur_prec && ass == LEFT))) {
                    break;
                }
            ...
            if (token->type == LPARENS) {
                ...
                free(token);
                while ((token = pop(&stack))->type != LPARENS) {
                    enqueue(&postfix, token);
                }
                free(token);
            }   
...
double evaluate_postfix(struct token_queue * pqueue_postfix) {
    ...
    union token_value value;
    p_expr_token stack = NULL, curr = NULL, operand_1 = NULL, operand_2 = NULL;
    while (curr = dequeue(pqueue_postfix)) {
        if (curr->type == OPERAND) {
            push(&stack, curr);
        } else if (curr->type == OPERATOR) {
            operand_1 = pop(&stack);
            if (op_operands[curr->value.op_code] == 1) {
                value.operand = -operand_1->value.operand;
            } else if (op_operands[curr->value.op_code] == 2) {
                operand_2 = pop(&stack);
                switch (curr->value.op_code) {
                    case ADD: {
                        value.operand = operand_2->value.operand + operand_1->value.operand;
                        break;
                    }
                    case SUBTRACT: {
                        value.operand = operand_2->value.operand - operand_1->value.operand;
            ...
            push(&stack, new_token(OPERAND, value));
            free(operand_1);
            free(curr);
        }
    }
    operand_1 = pop(&stack);
    double res = operand_1->value.operand;
    free(operand_1);
    return res;
}
