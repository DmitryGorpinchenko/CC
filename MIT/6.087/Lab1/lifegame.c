...
void initialize_world_from_file(const char * filename) {
    ...
    for(j = 0; j < WORLDHEIGHT && fgets(line, MAX_LINE_LEN, fp); j++) {
        for(i = 0; line[i] != '\0' && i < WORLDWIDTH; i++) {
...
int num_neighbors(int x, int y) {
    ...
    int count = 0;
    count += (get_cell_state(x - 1, y) + get_cell_state(x + 1, y));
    count += (get_cell_state(x, y - 1) + get_cell_state(x, y + 1));
    count += (get_cell_state(x - 1, y - 1) + get_cell_state(x + 1, y + 1));
    count += (get_cell_state(x - 1, y + 1) + get_cell_state(x + 1, y - 1));
    return count;
}
