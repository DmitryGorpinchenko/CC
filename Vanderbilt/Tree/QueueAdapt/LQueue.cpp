...
{
    if (free_list_) {
        auto head = free_list_;
        head->next_->prev_ = head->prev_;
        head->prev_->next_ = head->next_;
        free_list_ = free_list_->next_ == free_list_
                       ? nullptr 
                       : free_list_->next_;
        return head;
    }
    return ::new LQueue_Node();
}
...
{
    if (free_list_ == nullptr) {
        free_list_ = ::new LQueue_Node();
        --n;
    }
    for (size_t i = 0; i < n; ++i) {
        free_list_ = ::new LQueue_Node(free_list_, free_list_->prev_);
    }
}
...
template<typename T, typename LQUEUE_NODE> size_t LQueue<T, LQUEUE_NODE>::size() const { return count_; }
...
{
    if (this != &rhs) { LQueue tmp(rhs); swap(tmp); } return *this;
}
template <typename T, typename LQUEUE_NODE>
LQueue<T, LQUEUE_NODE>::~LQueue()
{
    while (!is_empty()) { dequeue_i(); } delete tail_;
}
...
template <typename T, typename LQUEUE_NODE> bool LQueue<T, LQUEUE_NODE>::is_empty() const { return size() == 0; }
template <typename T, typename LQUEUE_NODE> bool LQueue<T, LQUEUE_NODE>::is_full() const { return false; }
...
{
    auto tmp = *this; pos_ = pos_->next_; return tmp;
}
template <typename T, typename LQUEUE_NODE> LQueue_Iterator<T, LQUEUE_NODE> &LQueue_Iterator<T, LQUEUE_NODE>::operator--()
{
...
template <typename T, typename LQUEUE_NODE> LQueue_Iterator<T, LQUEUE_NODE> LQueue_Iterator<T, LQUEUE_NODE>::operator--(int)
{
    auto tmp = *this; pos_ = pos_->prev_; return tmp;
}
